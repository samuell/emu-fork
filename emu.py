...
# See CIGAR ops numbering here:
# https://pysam.readthedocs.io/en/latest/api.html#pysam.AlignedSegment.get_cigar_stats
CIGAR_OP_MATCH = 0
CIGAR_OP_INSERTION = 1
CIGAR_OP_DELETION = 2
CIGAR_OP_SOFTCLIP = 4
CIGAR_OP_NMTAG = 10

CIGAR_OPS = [CIGAR_OP_INSERTION, CIGAR_OP_DELETION, CIGAR_OP_SOFTCLIP, CIGAR_OP_NMTAG]
CIGAR_OPS_ALL = [CIGAR_OP_MATCH, CIGAR_OP_INSERTION, CIGAR_OP_DELETION, CIGAR_OP_SOFTCLIP]
...
def get_align_len(alignment):
    ...
    """
    # We access the first the first array here, as it contains the number of
    # nucleotides per CIGAR operation.
    # See docs here:
    # https://pysam.readthedocs.io/en/latest/api.html#pysam.AlignedSegment.get_cigar_stats
    nucleotides_per_cigar_op = alignment.get_cigar_stats()[0]

    # Here we count the nucletides for the CIGAR operations:
    # 0: Match
    # 1: Insertion
    # 2: Deletion
    # 4: Soft-clip
    align_len = sum(nucleotides_per_cigar_op[cigar_op] for cigar_op in CIGAR_OPS_ALL)
    return align_len
    ...
    for alignment in sam_pysam.fetch():
        ...
        if not alignment.is_secondary and not alignment.is_supplementary \
            ...
            if dict_longest_align[alignment.query_name] < align_len:
                old_align_len = dict_longest_align[alignment.query_name]
                new_align_len = align_len
                print(f"Updating shortest alignment for {alignment.reference_name}: {old_align_len} -> {new_align_len}")
                dict_longest_align[alignment.query_name] = align_len
...
def expectation_maximization(log_p_rgs, freq):
    """
    One iteration of the EM algorithm. Updates the relative abundance
    estimation in f based on probabilities in log_p_rgs.

    In each iteration, P(s|r), the probabilities of a read given a sequence, is
    calculated according to the current frequency vector.

    - First, L(r|s) * f(s) is calculated, then a fixed multiplier, C, is
      calculated for each read.
    - The fixed multiplier is then multiplied by L(r|s)*f(s), L(r|s) * f(s) * c.
    - Then sum(L(r|s) * f(s) * c) for each sequence is calculated.
    - Then (L(r|s) * f(s) * c) / (sum(L(r|s) * f(s) * c) for each sequence), is
      calculated.

    All calculations are done in log space. The frequency vector is then
    recalculated using the P(s|r) values, and the total log likelihood is
    updated.

    log_p_rgs({str:(int, float)}): dict[query_name]=(ref_tax_id, log(L(query_name|ref_tax_id)))
    freq{int:float}: dict[species_tax_id]: Likelihood that a species is present in the sample
    returns:
        f {int:float}: dict[species_tax_id]:updated likelihood species is present in sample
...
def expectation_maximization_iterations(log_p_rgs, db_ids, lli_thresh, input_threshold):
    ...
    Calls the expectation_maximization function during each iteration of the
    algorithm.  Stops iterations once the log likelihood is calculated to have
    increased less than threshold.
